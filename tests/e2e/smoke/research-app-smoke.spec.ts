...
test.describe('QA Scenario Lab smoke', () => {
  ...
  test('IndexedDBの初期データを使って一覧検索、詳細モーダル、実行結果登録を確認できる', async ({ page }) => {
    ...
    await page.getByLabel('検索キーワード').fill('詳細');
    await expect(page.getByRole('cell', { name: 'SCN-003' })).toBeVisible();
    await expect(page.getByRole('cell', { name: 'SCN-001' })).toHaveCount(0);

    await page.getByRole('button', { name: 'SCN-003 の詳細を開く' }).click();
    await expect(page.getByRole('dialog', { name: '詳細モーダルを表示できる' })).toBeVisible();
    await expect(page.getByText('期待結果')).toBeVisible();
    ...
    await page.getByLabel('対象シナリオ').selectOption('SCN-003');
    await page.getByLabel('担当者').fill('QA Agent');
    await page.getByLabel('実行結果').selectOption('Passed');
    await page.getByLabel('実行メモ').fill('IndexedDB保存のスモークテスト実行');
    await page.getByRole('button', { name: '登録' }).click();
  ...
  test('疑似ロールが閲覧者の場合は更新系操作が無効になる', async ({ page }) => {
    ...
    await page.getByLabel('疑似ロール').selectOption('閲覧者');
